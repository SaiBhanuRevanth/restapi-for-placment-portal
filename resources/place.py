...
class Phase(Resource):
    ...
    def post(self):
         ...
         parser.add_argument('stemail',type=str,required=True,help='stemail cannot be left blank')
         parser.add_argument('stphno',type=int,required=True,help='stphno cannot be left blank')
         parser.add_argument('currentsem',type=int,required=True,help='currentsem cannot be left blank')
         data=parser.parse_args()
         try:
...
class userupdate(Resource):
    ...
    def post(self):
        ...
        data=parser.parse_args()
        try:
            if data['dept_name']==None and data['first_name']==None and data['last_name']==None and data['gpa']==None and data['password']==None and data['dateregst']==None and data['stemail']==None and data['stphno']==None and data['currentsem']==None:
            ...
            if data['stemail']!=None:
                query(f"""update stdnt.stdetails set stemail='{data['stemail']}' where roll_no={data['roll_no']};""",return_json=False)
            if data['stphno']!=None:
                query(f"""update stdnt.stdetails set stphno={data['stphno']} where roll_no={data['roll_no']};""",return_json=False)
            if data['currentsem']!=None:
                query(f"""update stdnt.stdetails set currentsem={data['currentsem']} where roll_no={data['roll_no']};""",return_json=False)
